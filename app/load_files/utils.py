...
def transform_month_data_frame(month_df):
    ...
    account_id = month_df.get('account_currency').astype(
        str).str[:-1].astype(np.int64)
    currency_type = month_df.get('account_currency').astype(
        str).str[-1].astype(np.int64)
...
def process_balances_file(balances_file, file_type='monthly', selected_date=None, controls_to_run=None, params_control=None):
    ...
    if valid_data:
        ...
        balances_df.drop_duplicates(
            subset="account_id", keep=False, inplace=True)
